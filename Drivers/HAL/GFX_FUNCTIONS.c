...
{
    int           x, y, w = ST7735_WIDTH, h = ST7735_HEIGHT;
...
void testRects(uint16_t color)
{
    ...
    fillScreen(ST7735_BLACK);
    n     = min(ST7735_WIDTH, ST7735_HEIGHT);
    for (i = 2; i < n; i += 6) {
        i2 = i / 2;
...
{
    int x, y, w = ST7735_WIDTH, h = ST7735_HEIGHT, r2 = radius * 2;
...
{
    int           x, y, r2 = radius * 2,
                        w = ST7735_WIDTH  + radius,
                        h = ST7735_HEIGHT + radius;
...
void testRoundRects() {
    ...
    fillScreen(ST7735_BLACK);
    w     = min(ST7735_WIDTH, ST7735_HEIGHT);
    red = 0;
    step = (256 * 6) / w;
    for (i = 0; i < w; i += 6) {
        i2 = i / 2;
        red += step;
        drawRoundRect(cx - i2, cy - i2, i, i, i / 8, ST7735_COLOR565(red, 0, 0));
    }
...
void testFilledRoundRects() {
    ...
    fillScreen(ST7735_BLACK);
    green = 256;
    step = (256 * 6) / min(ST7735_WIDTH, ST7735_HEIGHT);
    for (i = min(ST7735_WIDTH, ST7735_HEIGHT); i > 20; i -= 6) {
        i2 = i / 2;
        green -= step;
        fillRoundRect(cx - i2, cy - i2, i, i, i / 8, ST7735_COLOR565(0, green, 0));
    }
