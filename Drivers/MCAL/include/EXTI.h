...
typedef struct
{
	uint16_t      EXTI_LineNumber;    //@ref EXTI_line
...
typedef struct
{
	...
	EXTI_GPIO_Mapping_t EXTI_Pin;    //specifies the mapping
								//@ref EXTI_define

	uint8_t        TriggerCase;  //specify case
								//@ref trigger_define

	uint8_t        IRQ_EN;      //enable or dis
								//@ref ENABLE
	uint8_t		   PULL_RES;   // PULL UP OR DOWN OR NONE
								//@ref pull_res
	void (*P_IRQ_CallBack)(void);  //set the function for interrupt handling
...
#define EXTI_TriggerRising                     0
#define EXTI_TriggerFalling                    1
#define EXTI_TriggerRisingAndFalling           2
...
// @ref pull_res
#define EXTI_PULL_RES_UP                   GPIO_MODE_INPUT_PU
#define EXTI_PULL_RES_DN				   GPIO_MODE_INPUT_PD
#define EXTI_PULL_RES_NONE				   GPIO_MODE_INPUT_FLO
