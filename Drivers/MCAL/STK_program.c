...
Std_ReturnType STK_SetDelayAny_ms(f32 Copy_Milliseconds)
{
	/**< Calculate the number of ticks required to wait for the specified number of milliseconds */
	u32 Local_u32Ticks = (u32)((Copy_Milliseconds * STK_AHB_CLK) / 1000.0);
	...
	else if (Local_u32Ticks > 0x00FFFFFF)
	{
		...
		{
			/**< Configure SysTick timer with the MAX number of ticks */
			STK->LOAD = 0x00FFFFFF;

			/**< update number of ticks for the nexr overflow cycle */
		...
		/**< Configure SysTick timer with the remaining number of ticks */
		STK->LOAD = Local_u32Ticks;
...
Std_ReturnType STK_SetDelayAny_us(f32 Copy_Microseconds)
{
